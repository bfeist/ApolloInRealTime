...
var gTelemetryData = [];
var gCrewStatusData = [];
// var gOrbitData = []; //unneeded for 13
var gPhotoData = [];
var gPhotoIndex = [];
...
function findClosestUtterance(secondsSearch) {
  ...
  trace(
    "findClosestUtterance(): searched utterance array, found closest: timeid" +
      gUtteranceIndex[i - 1] +
      " after " +
      i +
      " searches"
...
function populatePhotoGallery() {
  ...
  for (var i = 0; i < gPhotoIndex.length; i++) {
    ...
    if (filetypematch !== null) {
      ...
      //if 'supporting' filename is in datafile then use AIRT image
...
function showPhotoByTimeId(timeId) {
  ...
  if (gPhotoDataLookup.hasOwnProperty(timeId)) {
    ...
    var scrollDest =
      photoGalleryDiv.scrollTop() + $(photoGalleryImageTimeId).offset().top - gNavigatorHeight - 75; //added offset for ted design
...
function updateDashboard(timeId) {
  ...
  if (
    timeIdInSeconds < timeStrToSeconds("075:06:08") ||
    timeIdInSeconds > timeStrToSeconds("085:59:00")
  ) {
    //earth reference range
    velocityIndex = 1;
    distanceIndex = 2;
  ...
  if (calculateVelocity) {
    if (timeIdInSeconds < timeStrToSeconds("142:54:40")) {
      //splashdown time
  ...
  if (calculateDistanceFromEarth) {
    if (timeIdInSeconds > 0 && timeIdInSeconds < timeStrToSeconds("142:54:40")) {
      //A13 splashdown time
      for (counter = 0; counter < gTelemetryData.length; counter++) {
        if (timeStrToSeconds(gTelemetryData[counter][0]) < timeIdInSeconds) {
  ...
  for (var counter = 0; counter < gVideoSegments.length; counter++) {
    if (
      ...
      if (gVideoSegments[counter][2] === "3D") {
        if (LROOverlaySelector.is(":hidden")) graphOverlaySelector.fadeOut();
        LROOverlaySelector.fadeIn();
      } else if (gVideoSegments[counter][2] === "graph") {
        LROOverlaySelector.fadeOut();
        graphOverlaySelector.fadeIn();
      } else {
        if (LROOverlaySelector.is(":visible")) LROOverlaySelector.fadeOut();
        if (graphOverlaySelector.is(":visible")) graphOverlaySelector.fadeOut();
      }
      //hide dashboard overlay if it is displayed (once per video segment)
      if (
        dashboardOverlaySelector.css("display").toLowerCase() !== "none" &&
...
jQuery(function ($) {
  ...
  $("#shareBtn").click(function () {
    ...
    if (gMOCRToggled) {
      var url =
        "https://apolloinrealtime.org/13/?t=" +
        gCurrMissionTime +
        "&ch=" +
